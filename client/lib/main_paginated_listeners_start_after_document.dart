...
class MyApp extends StatelessWidget {
  final MessagesWithPaginatedListenersStartAfterDocumentStrategy _messages;
  MyApp()
      : _messages = MessagesWithPaginatedListenersStartAfterDocumentStrategy(
            MessagesRepository());
  @override
...
class MessageList extends StatelessWidget {
  static Provider inController(
      MessagesWithPaginatedListenersStartAfterDocumentStrategy messages) {
...
class MessagesWithPaginatedListenersStartAfterDocumentStrategy {
  static const _partitionSize = 10;
  ...
  MessagesWithPaginatedListenersStartAfterDocumentStrategy(this._repository) {
    _initialize();
  }
    ...
    _paginationController.stream.exhaustMap((_) async* {
      if (_hasLastBatch) {
        ...
        final messageBatch = Messages(_repository, _partitionSize,
            startAfterPosition: _paginatedMessages.value.last.lastMessage);
...
class Messages {
  ...
  final Message _startAfterPosition;

  StreamSubscription<List<DataChange<Message>>> _subscription;

  Messages(this._repository, this._partitionSize, {Message startAfterPosition})
  ...
  Message get lastMessage => _messages.value.last;
  ...
  void _listen() async {
    ...
    _subscription = (await _repository.changes(_partitionSize,
            startAfter: _startAfterPosition))
        .listen((dataChanges) {
...
class MessagesRepository {
  Future<Stream<List<DataChange<Message>>>> changes(int limitNumber,
      {Message startAfter}) async {
    ...
    return query.snapshots().map((qs) {
      querySnapshotDebugPrint(
          qs, 'result of start after document ${startAfter.id}');
